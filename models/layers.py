...
class GraphLayer(nn.Module):
    ...
    def forward(self, code_x, neighbor, c_embeddings, n_embeddings):
        ...
        cc_embeddings = center_codes * torch.matmul(self.adj, center_embeddings) # sum of adjacent nodes' embeddings code_num x code_size
        cn_embeddings = center_codes * torch.matmul(self.adj, neighbor_embeddings) # sum of undiagnosed neighbor nodes' embeddings code_num x code_size
...
class TransitionLayer(nn.Module):
    ...
    def forward(self, t, co_embeddings, divided, no_embeddings, unrelated_embeddings, hidden_state=None):
        ...
        m2_index = torch.where(m2 > 0)[0] # disease id that appear in visit t and is an undiagnosed neighbor in visit t-1
        ...
        if len(m1_index) > 0: # embedding of persistent diseases
            ...
            h = hidden_state[m1_index] if hidden_state is not None else None # select corresponding hidden states, len(m1_index) x hidden_size
        ...
        if t > 0 and len(m2_index) + len(m3_index) > 0: # embedding of emerging disease
            q = torch.vstack([no_embeddings[m2_index], unrelated_embeddings[m3_index]]) # (len(m2_index) + len(m3_index)) x graph_size, neighbor embeddings of last visit + unrelated embeddings of last visit
            v = torch.vstack([co_embeddings[m2_index], co_embeddings[m3_index]]) # (len(m2_index) + len(m3_index)) x graph_size, diagnosed emerging disease embeddings of this visit
